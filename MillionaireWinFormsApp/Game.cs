...
namespace MillionaireWinFormsApp
{
    ...
    public class Game
    {
        ...
        public Question FiftyFifty()
        {
            ...
            var noCorrectAnswers = currentQuestion.Answers.Where(x => !x.IsCorrect).ToArray();

            var random = new Random();
            var randomIndex = random.Next(noCorrectAnswers.Length);

            for (int i = 0; i < noCorrectAnswers.Length; i++)
            {
                if (randomIndex != i)
                {
                    noCorrectAnswers[i].Text = string.Empty;
                }
            }
